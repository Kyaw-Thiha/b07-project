...
Running `TestAPIActivity` now seeds Firebase Auth with a fixed trio of debug accounts, so you always log in with the same credentials after generating data:

| User type | UID | Email | Password |
|-----------|-----|-------|----------|
| Parent | `debug-parent-01` | `debug-parent@seed-users.example.com` | `ParentPass123!` |
| Child | `debug-child-01` | `debug-child@seed-users.example.com` | `ChildPass123!` |
| Provider | `debug-provider-01` | `debug-provider@seed-users.example.com` | `ProviderPass123!` |

The script reuses these accounts if they already exist, so you can run it multiple times without hitting “email already in use.” Just sign in with the credentials above for whichever role you need to test.
